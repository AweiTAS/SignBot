...
class FastSAMPrompt:
    ...
    def mergeIntersectAppend(self, closeEnough_list, index):
        if(len(closeEnough_list)==0):
            closeEnough_list.append(index)
            return closeEnough_list

        mask1 = np.uint8(self._format_results(self.results[0], 0)[index]['segmentation'])

        for i in range(len(closeEnough_list)):
            mask2 = self._format_results(self.results[0], 0)[i]['segmentation']
            if((mask1 * mask2).any()>0):
                mask2 = mask1 + mask2
                return closeEnough_list
    ...
    def color_prompt(self, RGBcolor, maxDis, secondMaxDis):
        RGBcolor = np.uint8([[RGBcolor]])
        targetColor = cv2.cvtColor(RGBcolor, cv2.COLOR_RGB2YCrCb)[0][0]
        masks = self._format_results(self.results[0], 0)
        closeEnough_index = []
        length = len(masks)
        YCbCrImg = cv2.cvtColor(self.img, cv2.COLOR_RGB2YCrCb)
        for curMask in range(length):
            localImg = np.copy(YCbCrImg)
            j = masks[curMask]['segmentation']
            mask = np.dstack((j,j,j))
            localImg = localImg * mask
            colorAvg = np.sum(np.sum(localImg, axis=0), axis=0)/masks[curMask]['area']
            closeEnough = np.linalg.norm(colorAvg[1:3] - targetColor[1:3]) <= maxDis
            if closeEnough:
                closeEnough_index = self.mergeIntersectAppend(closeEnough_index, curMask)
                #closeEnough_index.append(curMask)
        self.img = cv2.cvtColor(self.img, cv2.COLOR_RGB2GRAY)
        if len(closeEnough_index) == 0:
            print("no object")
        #else:
            #print("There are " + str(len(closeEnough_index)) + " masks!")
        #closeEnough_index = list(set(closeEnough_index)) #make sure there is not duplicate
        masks = self.results[0].masks.data
        return np.array(masks[closeEnough_index].cpu().numpy())